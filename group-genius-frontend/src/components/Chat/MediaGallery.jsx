...
const MediaGallery = ({ groupId, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center p-4">
      <div className="w-full max-w-6xl bg-white dark:bg-slate-900 rounded-lg shadow-lg overflow-hidden">
        <div className="flex items-center justify-between p-4 border-b dark:border-slate-700">
          <div className="flex items-center gap-2">
            <h3 className="text-lg font-semibold">Media</h3>
            <div className="text-sm text-slate-500">{downloads.length} items</div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={onClose} className="ml-2 p-2 rounded hover:bg-slate-100 dark:hover:bg-slate-800"><X className="w-5 h-5" /></button>
          </div>
        </div>

        <div className="p-4">
          {filteredDownloads.length === 0 ? (
            <div className="text-center text-slate-500 py-24">No media found</div>
          ) : activeTab === "images" ? (
            <div className="space-y-6">
              {photos.length > 0 && (
                <div>
                  <h4 className="text-sm font-semibold text-slate-600 dark:text-slate-300 mb-2">Photos</h4>
                  {renderGalleryGrid(photos)}
                </div>
              )}
              {gifs.length > 0 && (
                <div>
                  <h4 className="text-sm font-semibold text-slate-600 dark:text-slate-300 mb-2">GIFs</h4>
                  {renderGalleryGrid(gifs)}
                </div>
              )}
              {videos.length > 0 && (
                <div>
                  <h4 className="text-sm font-semibold text-slate-600 dark:text-slate-300 mb-2">Videos</h4>
                  {renderGalleryGrid(videos)}
                </div>
              )}
            </div>
          ) : (
              ...
              {filteredDownloads.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <a href={item.url} download={item.name} className="p-2 hover:bg-gray-200 rounded-full transition" onClick={(e) => e.stopPropagation()}>
      ...
      {selectedMedia && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4" onClick={closeMediaViewer}>
          <div className="relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl max-w-2xl w-full overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <button onClick={closeMediaViewer} className="absolute top-4 right-4 p-2 bg-white/90 dark:bg-slate-800/90 hover:bg-white dark:hover:bg-slate-700 rounded-full transition-all duration-200 text-gray-700 dark:text-gray-200 z-10 shadow-lg hover:scale-110" aria-label="Close viewer">
               <X className="w-6 h-6" />
             </button>

             {mediaItems.length > 1 && (
               <>
                <button onClick={(e) => { e.stopPropagation(); navigateMedia(-1); }} className="absolute left-4 top-1/2 -translate-y-1/2 p-2.5 bg-white/90 dark:bg-slate-800/90 hover:bg-white dark:hover:bg-slate-700 rounded-full transition-all duration-200 text-gray-700 dark:text-gray-200 z-10 shadow-lg hover:scale-110" aria-label="Previous">
                   <ChevronLeft className="w-6 h-6" />
                 </button>
                <button onClick={(e) => { e.stopPropagation(); navigateMedia(1); }} className="absolute right-4 top-1/2 -translate-y-1/2 p-2.5 bg-white/90 dark:bg-slate-800/90 hover:bg-white dark:hover:bg-slate-700 rounded-full transition-all duration-200 text-gray-700 dark:text-gray-200 z-10 shadow-lg hover:scale-110" aria-label="Next">
                   <ChevronRight className="w-6 h-6" />
                 </button>
               </>
             )}

            <div className="flex items-center justify-center p-8 max-h-[70vh] overflow-hidden bg-gradient-to-b from-slate-50 to-white dark:from-slate-800 dark:to-slate-900">
            ...
            <div className="px-6 py-4 border-t dark:border-slate-700 bg-white dark:bg-slate-900">
              <p className="font-semibold text-gray-900 dark:text-white truncate text-sm mb-2">{selectedMedia.name}</p>
