...
public class UserController {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<User> updateUser(@PathVariable Long id, @RequestBody UserDto updateRequest) {
    ...
    public ResponseEntity<User> uploadAvatar(@PathVariable Long id, @RequestParam("avatar") MultipartFile avatar) throws IOException {
        return ResponseEntity.ok(profileService.uploadAvatar(id, avatar));
    }

    @PostMapping("/{userId}/courses/{courseId}")
    public ResponseEntity<User> addCourse(@PathVariable Long userId, @PathVariable Long courseId) {
        return ResponseEntity.ok(userService.addCourse(userId, courseId));
    }

    @DeleteMapping("/{userId}/courses/{courseId}")
    public ResponseEntity<User> removeCourse(@PathVariable Long userId, @PathVariable Long courseId) {
        return ResponseEntity.ok(userService.removeCourse(userId, courseId));
    }

    @PutMapping("/{userId}/courses/{courseId}")
    public ResponseEntity<User> updateCourse(@PathVariable Long userId, @PathVariable Long courseId, @RequestParam("active") boolean active) {
